...
```{r}
authentication <- list(consumer_key = "4VwU8uha9npxxduK4dalXiysh",
                 consumer_secret = "25G9H5Gt4E7EYl74CDCIuWOwxw01X2d8fFZsKUV39JNO3jU5jG",
                 access_token = "1694784686-A4S3VxWWLxeJBVWaX3XK7GR9X5MuirEPbjlfbWI",
                 access_token_secret = "1fnx9vWdy7mKdTvSpqPPZv5mp7AB8qCQcefm07Adh9apC")
```
...
```{r}
sample_tweets <- stream_tweets(q = "", timeout = 30, parse=FALSE, file_name = "streamed_tweets.json")
sample_tweets <- tryCatch({parse_stream("streamed_tweets.json")}, error = function(e)
    {print(paste("Retrying with alteranative function after initial error when parsing file", "streamed_tweets.json")); 
    return(recover_strem("streamed_tweets.json"))})
head(sample_tweets)
...
```{r}
keywords_tweets <- stream_tweets("BTS OR Biden", timeout = 5)
